...
if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
  ...
  ${B}B) SSH anahtarı — token hiç kullanılmaz${OFF}
       ...
       git remote set-url $REMOTE git@github.com:m3sto/openos.git
...
echo
echo "  Pano:  ${B}https://m3sto.github.io/openos/${OFF}"
echo "  ${DIM}Pages açık değilse: Settings → Pages → Deploy from a branch → main / (root)${OFF}"
